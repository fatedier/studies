		...
		ConnectionInfo *pNewStorage, bool *new_connection)
{
	...
	/* 如果ppStorageServer没有传，从pTrackerServer上获取一台源storage建立连接 */
	...
	/* 如果ppStorageServer传了，建立同这台storage的连接 */
	else
	{
...
{
	char new_group_name[FDFS_GROUP_NAME_MAX_LEN + 1];	//Storage Server所在的组名
	char remote_filename[128];	//存放在Storage Server上的文件名
	int result;
	...
	{
		snprintf(new_group_name, sizeof(new_group_name), 	//如果已经指定Storage的组名
			"%s", group_name);
	}
	...
	if (result == 0)		//上传成功，file_id格式为 "组名/文件名"，文件名包含了路径
	{
	...
	char *group_name, char *remote_filename)
{
	...
	char *pInBuff;
	ConnectionInfo storageServer;
	bool new_connection;		//是否是获取的新的StorageServer的连接
	bool bUploadSlave;
	int new_store_path;
	...
	*remote_filename = '\0';
	...
	else if ((result=storage_get_upload_connection(pTrackerServer, 	//获取一个StorageServer服务器的连接，没有就获取一个新的
	...
	*/
	...
	if (new_connection)		//如果是获取的新的StorageServer的连接，断开与之的连接
	{
		tracker_disconnect_server_ex(pStorageServer, result != 0);
	}
		...
		const int meta_count, char *group_name, char *remote_filename)
{
	...
	if (stat(local_filename, &stat_buf) != 0)	//获取该文件的状态信息，存放在stat_buf中
	{
		group_name[0] = '\0';
		remote_filename[0] = '\0';
		return errno;
	...
	if (!S_ISREG(stat_buf.st_mode))		//根据st_mode判断是否是一个常规文件
	{
		group_name[0] = '\0';
		remote_filename[0] = '\0';
		return EINVAL;
	...
	if (file_ext_name == NULL)
	{
		file_ext_name = fdfs_get_file_ext_name(local_filename);	//根据文件名获取文件扩展名
	}
 ...
 * 获取一台用于下载文件storage的连接
 * 如果ppStorageServer没有传，向pTrackerServer发送报文获取一台storage用于连接
 * 之后向此storage发送下载文件到本地的报文，接收到文件内容后执行callback回调函数
 */
