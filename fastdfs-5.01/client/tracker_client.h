...
/* 获取和所有tracker中的一个的连接，连接信息在pTrackerServer中返回 */
...
**/
/* 
 * 向tracker发送报文，获取指定group_name的group的状态信息，在pDest中返回
 * 发送报文:group_name 
...
**/
/* 
 * storage向tracker发送获取指定group和ip地址的storage的状态信息 
 * 报文体:group_name，ip地址(可有可无，如果没传，从当前连接的客户端取ip) 
 * 返回报文体:要查询的storage的FDFSStorageBrief对象信息，放在pDestBuff中返回
 */
