...
/* 添加新的tracker_server信息到排序好的tracker_group中 */
...
/* 将配置文件中配置的tracker_server信息加载到内存中的pTrackerGroup对象中 */
		...
		const char *filename, char **ppTrackerServers)
{
	...
	for (ppSrc=ppTrackerServers; ppSrc<ppEnd; ppSrc++)
	{
		...
		/* 如果配置的是新的tracker_server，加入到tracker_group中 */
