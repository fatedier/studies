...
static int storage_check_and_make_data_dirs()
{
	...
	snprintf(data_path, sizeof(data_path), "%s/data", \
			g_fdfs_base_path);
	/* data目录下 .data_init_flag文件 */
	snprintf(full_filename, sizeof(full_filename), "%s/%s", \
			data_path, DATA_DIR_INITED_FILENAME);
	/* 如果不是第一次启动了，读取.data_init_flag文件 */
	if (fileExists(full_filename))
	{
		IniContext iniContext;
		char *pValue;
	...
	/* 
	 * 在每一个指定的路径下创建data目录以及二级、三级存储目录
	 * 如果需要恢复 文件的，从同组的其他storage同步
	 */
	for (i=0; i<g_fdfs_store_paths.count; i++)
	{
		/* 在指定路径目录下创建data目录，并且按照设置创建二级和三级存储目录 */
...
/* 在pBasePath目录下创建data目录，并且按照设置创建二级和三级存储目录 */
static int storage_make_data_dirs(const char *pBasePath, bool *pathCreated)
{
	char data_path[MAX_PATH_SIZE];
	char dir_name[9];
	...
	*pathCreated = false;
	...
	/* 存储目录命名规则为从min_sub_path到max_sub_path，16进制 */
	...
	/* 
	 * 在当前data目录下
	 * 创建g_subdir_count_per_path个二级目录
	 * 然后在每个二级目录下创建g_subdir_count_per_path个三级目录 
	 */
...
/* 从storage配置文件中获取相关路径信息，包括base_path, store_path等 */
		...
		char *bind_addr, const int addr_size)
{
	...
	*/
	...
	do
	{
		...
		/* 获取并加载配置文件设置的所有tracker server的信息 */
		...
		*/
		...
		/* access log按文件大小轮转，达到这么大时就会轮转，为0不轮转 */
		...
		/* error log按文件大小轮转，达到这么大时就会轮转，为0不轮转 */
