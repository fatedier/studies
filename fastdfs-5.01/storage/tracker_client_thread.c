...
static pthread_mutex_t reporter_thread_lock;	/* reporter线程互斥锁 */

/* save report thread ids */
static pthread_t *report_tids = NULL;
static int *src_storage_status = NULL;  /* returned by tracker server(源storage的状态信息) */
...
/* 删除用于sync和trunk的mark_file (增加当前时间作为后缀) */
...
/* 根据新的ip和port修改用于sync和trunk的mark_file的文件名 */
	...
	while (g_continue_flag)
	{
		...
		*/
		...
		if (!sync_old_done)
		{
			...
			if (!g_sync_old_done)
			{
				/* 向tracker server发送报文获取一个用于同步的源storage server */
		...
		while (g_continue_flag)
		{
					...
					g_heart_beat_interval)
			{
				...
				/* 
				 * 如果需要自动调整ip地址
				 * 向tracker server发送获取changelog的请求报文 
				 */
			...
			{
				/* 向tracker发送报文，汇报自己的storage磁盘的使用状态 */
			...
			if (g_if_trunker_self)
			{
				...
				{
					/* storage向tracker server发送报文汇报当前的trunk_file_id */
				...
				{
					/* storage向tracker server发送报文汇报trunk的空闲空间 */
...
/* 将briefServers同g_sorted_storages进行合并 */
	...
	bool *bServerPortChanged)
{
	...
	if ((*pFlags) & FDFS_CHANGE_FLAG_TRUNK_SERVER)
	{
		...
		*(g_trunk_server.ip_addr + (IP_ADDRESS_SIZE - 1)) = '\0';
			...
			g_trunk_server.port == g_server_port)
		{
			...
			/* 向tracker server发送报文获取当前的trunk_file_id */
	...
	/* 将pBriefServers同g_sorted_storages进行合并 */
...
{
#define FDFS_CHANGELOG_FIELDS  5	/* changelog文件一行的列数 */
	int64_t nInPackLen;
	char *pInBuff;
	char *pBuffEnd;
	char *pLineStart;
	...
	*(pInBuff + nInPackLen) = '\0';
	...
	while (pLineStart < pBuffEnd)
	{
		...
		do
		{
			...
			{
				/* 删除和此storage相关的用于sync和trunk的mark_file (增加当前时间作为后缀) */
