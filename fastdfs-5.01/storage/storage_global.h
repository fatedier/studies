...
#define STORAGE_BEAT_DEF_INTERVAL    30		/* 默认发送心跳包的间隔时间 */
#define STORAGE_REPORT_DEF_INTERVAL  300		/* 默认汇报状态的间隔时间 */
#define STORAGE_DEF_SYNC_WAIT_MSEC   100
#define DEFAULT_DISK_READER_THREADS  1
#define DEFAULT_DISK_WRITER_THREADS  1
#define DEFAULT_SYNC_STAT_FILE_INTERVAL  300		/* 同步状态文件到磁盘的间隔时间 */
#define DEFAULT_DATA_DIR_COUNT_PER_PATH	 256		/* 默认每个路径下的目录数量 */
#define DEFAULT_UPLOAD_PRIORITY           10			/* storage默认上传文件的优先级 */
/* 同步完多少个文件后，把storage的mark文件同步到磁盘 */
#define FDFS_DEFAULT_SYNC_MARK_FILE_FREQ  500
#define STORAGE_DEFAULT_BUFF_SIZE    (64 * 1024)		/* 默认发送和接收数据的最大长度 */

#define STORAGE_FILE_SIGNATURE_METHOD_HASH  1	/* 文件签名方式，哈希 */
#define STORAGE_FILE_SIGNATURE_METHOD_MD5   2		/* 文件签名方式，MD5 */
...
} FDFSStorePathInfo;	/* storage下每个存储路径的信息 */
