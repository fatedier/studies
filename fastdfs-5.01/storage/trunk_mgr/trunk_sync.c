...
static int trunk_binlog_fd = -1;		/* trunk_binlog_file的文件描述符 */

int g_trunk_sync_thread_count = 0;		/* 正在运行中的trunk同步线程数 */
static pthread_mutex_t trunk_sync_thread_lock;
static char *trunk_binlog_write_cache_buff = NULL;	/* trunk_binlog文件的写入缓存 */
static int trunk_binlog_write_cache_len = 0;			/* trunk_binlog文件的写入缓存的大小 */
static int trunk_binlog_write_version = 1;			

/* save sync thread ids */
static pthread_t *trunk_sync_tids = NULL;		/* 保存trunk同步线程的tid数组 */
...
/*缓冲区的内容写入trunk_binlog_file，并强制刷新到磁盘 */
...
static char *get_trunk_rollback_filename(char *full_filename)
{
	...
	/* trunk_binlog_file加上.roallback后缀面 */
	strcat(full_filename, TRUNK_SYNC_BINLOG_ROLLBACK_EXT);
	return full_filename;
}

/* 以只写方式打开trunk_binlog_file */
...
int trunk_binlog_compress_apply()
{
	...
	/* 如果没有打开trunk_binlog_file，重命名为rollback_filename之后返回 */
	if (trunk_binlog_fd < 0)
	{
...
/* 将old_fd中的内容和trunk_binlog_file中的内容合并放入trunk_binlog_file中 */
static int trunk_binlog_merge_file(int old_fd)
{
	...
	/* 将old_fd中的内容写入trunk_binlog_file.tmp临时文件 */
	...
	if (access(binlog_filename, F_OK) == 0)
	{
		...
		/* 将原来的trunk_binlog_file文件的内容写入trunk_binlog_file.tmp临时文件 */
...
int trunk_binlog_compress_commit()
{
	...
	/* 将storage_trunk.dat中的内容和trunk_binlog_file中的内容合并放入trunk_binlog_file中 */
...
/* buff中的内容写入trunk_binlog_file，并强制刷新到磁盘 */
static int trunk_binlog_fsync_ex(const bool bNeedLock, \
		const char *buff, int *length)
{
	int result;
		...
		const FDFSTrunkFullInfo *pTrunk)
{
	...
	/* 如果缓冲区满，将缓冲区的内容写入trunk_binlog_file，并强制刷新到磁盘 */
		...
		char *full_filename)
{
	...
	/* 文件名为 base_path/data/trunk/binlog */
	snprintf(full_filename, MAX_PATH_SIZE, 
		"%s/data/"TRUNK_DIR_NAME"/"TRUNK_SYNC_BINLOG_FILENAME, \
		g_fdfs_base_path);
	return full_filename;
		...
		const int port, char *full_filename, const int filename_size)
{
	if (g_use_storage_id)
	{
		/* 路径为/data/trunk/(storage_id).mark */
...
int trunk_reader_init(FDFSStorageBrief *pStorage, TrunkBinLogReader *pReader)
{
	...
	/* 根据trunk_reader信息(id和port)获取trunk的mark_file */
	...
	/* 如果mark_file不存在，将当前trunk_binlog_reader信息写入mark_file */
...
/* 将当前trunk_binlog_reader信息写入mark_file */
static int trunk_write_to_mark_file(TrunkBinLogReader *pReader)
{
	char buff[128];
	int len;
...
/* 读取trunk_binlog_file文件中的一行，先从缓冲区中读 */
...
/* 结束自身线程，g_trunk_sync_thread_count-1，并且从trunk_sync_tids中删除 */
...
/* 将从binlog_file中读取出的pReader->buff中的内容同步到指定的storage */
...
static void* trunk_sync_thread_entrance(void* arg)
{
		...
		pStorage->status != FDFS_STORAGE_STATUS_NONE)
	{
		...
		*/
		...
		sync_result = 0;
		/* 循环将trunk_binlog_file中的内容传送给指定的storage */
...
int trunk_unlink_all_mark_files()
{
		...
		pStorageServer++)
	{
		...
		/* 将指定storage_id对应的mark_file重命名，加上当前时间作为后缀，相当于删除 */
