...
FDFSStorePaths g_fdfs_store_paths = {0, NULL};		/* storage的数据存储目录 */
		...
		int *filename_len, char *true_filename, char **ppStorePath)
{
	...
	/* 解析logic_filename，store_path_index为M后面的两位，并且true_filename减去四个字节 "M00/" */
	SPLIT_FILENAME_BODY(logic_filename, filename_len, true_filename, \
		store_path_index, true);

	/* ppStorePath指向store_path_index对应的路径名 */
	...
	FDFSTrunkHeader *pTrunkHeader, int *pfd)
{
	...
	do
	{
		/* 获取trunk_file中对应的链接到的源文件名，存放在src_filename中 */
	...
	FDFSTrunkHeader *pTrunkHeader, int *pfd)
{
	...
	/* 对true_filename解密，解析出FDFSTrunkFileInfo的id，offset，size相关信息 */
