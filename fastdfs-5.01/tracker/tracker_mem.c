...
#define TRACKER_MEM_ALLOC_ONCE	2		/* 每一次分配新增的内存对象个数 */

/* storage_groups_new.dat文件中的字段名 */
...
TrackerServerGroup g_tracker_servers = {0, 0, -1, NULL};		/* 所有tracker_server的信息 */
ConnectionInfo *g_last_tracker_servers = NULL;  /* for delay free(缓存tracker信息变更的上一次状态) */
int g_next_leader_index = -1;			   //next leader index
int g_trunk_server_chg_count = 1;		   /* for notify other trackers(trunk server变更的次数) */
int g_tracker_leader_chg_count = 0;		   /* for notify storage servers(tracker leader变更的次数) */

int64_t g_changelog_fsize = 0; //storage server change log file size(changelog文件的大小)
static int changelog_fd = -1;  //storage server change log fd for write(changelog文件描述符)
static bool need_get_sys_files = true;	/* 是否需要从其他tracker server获取配置文件 */
static bool get_sys_files_done = false;	/* 获取配置文件是否完成 */

static pthread_mutex_t mem_thread_lock;	/* tracker内存对象的互斥锁 */
static pthread_mutex_t mem_file_lock;		/* 读写tracker相关文件的互斥锁 */

static void tracker_mem_find_store_server(FDFSGroupInfo *pGroup);

/* 
 * 选取trunk_binlog_size最大的storage server设为trunk server 
 * save为true，表示需要将group信息写入文件中
 */
...
/* 如果指定的gid或者uid和全局变量中的不一样，修改指定文件所有者为指定值 */
...
/* 将group中的pStorage的改变信息写入changlog文件 */
...
/* 分配每一个storage中的store_path相关的变量的内存，例如path_total_mbs等 */
...
/* 分配所有groups中的每一个storage中的store_path相关的变量的内存，例如path_total_mbs等 */
...
#define STORAGE_DATA_GROUP_FIELDS	4
	...
	FILE *fp;
	char szLine[256];		/* 存储一行信息，每行不超过255字符 */
	char group_name[FDFS_GROUP_NAME_MAX_LEN + 1];
	char *fields[STORAGE_DATA_GROUP_FIELDS];		/* 存放每一行数据的每一列数据 */
	int result;
	int col_count;		/* 每一行配置的列数 */
	TrackerClientInfo clientInfo;
	bool bInserted;

	/* 以只读方式打开groups信息old文件 */
	...
	while (fgets(szLine, sizeof(szLine), fp) != NULL)
	{
		...
		/* 第2个字段是storage server与tracker server通讯的端口 */
		...
		{
			/* 第3个字段是每个storage server的base path的个数 */
			clientInfo.pGroup->store_path_count = \
				atoi(trim(fields[2]));
			/* 第4个字段是每个storage server的base path下的二级目录的个数，总个数是此数的平方 */
...
/* 
 * 读取groups_new中的内容
 * 如果只有groups_old文件，读取里面的内容后写入新的groups_new文件
 */
		...
		FDFSStorageSync **ppTrunkServers, int *nTrunkServerCount)
{
	...
	*nTrunkServerCount = 0;
	*ppTrunkServers = NULL;
	/* 如果只有groups_old文件，读取内容，并用groups_new文件覆盖 */
...
/* 根据groups中的group_name和storage_server_id定位到实际的trunk_server对象 */
		...
		const int nStorageSyncCount, const bool bLoadFromFile)
{
	...
	{
		/* 根据storage所在group的group_name获取group */
		pGroup = tracker_mem_get_group_ex(pGroups, \
...
/* 
 * 读取storage的配置文件
 * 如果有旧的storage配置文件，从旧的加载，之后保存进新的storage配置文件
 * 否则直接从新的storage配置文件加载
 */
...
int tracker_save_groups()
{
	...
	/* 如果groups内容写入文件成功，将临时文件名改为真正的文件名 */
	if (result == 0)
	{
...
int tracker_save_sync_timestamps()
{
	...
	/* 将同步时间戳写入data/storage_sync_timestamp.dat文件 */
	snprintf(trueFilename, sizeof(trueFilename), "%s/data/%s", \
...
/* 打开changelog file，如果不存在，就创建文件并修改属主 */
static int tracker_open_changlog_file()
{
	char data_path[MAX_PATH_SIZE];	/* fdfs根目录下的data目录 */
	char filename[MAX_PATH_SIZE];

	snprintf(data_path, sizeof(data_path), "%s/data", g_fdfs_base_path);
	/* 检查data目录是否存在，如果不存在，创建目录，修改属主 */
	if (!fileExists(data_path))
	{
		if (mkdir(data_path, 0755) != 0)
		{
...
static int tracker_mem_init_groups(FDFSGroups *pGroups)
{
	int result;
	FDFSGroupInfo **ppGroup;
	FDFSGroupInfo **ppGroupEnd;		/* 指向groups中最后一个group的指针 */

	/* 初始化组信息 */
	pGroups->alloc_size = TRACKER_MEM_ALLOC_ONCE;	/* 初始分配内存对象个数 */
	pGroups->count = 0;
	pGroups->current_write_group = 0;
	pGroups->pStoreGroup = NULL;
	/* groups存放所有group的结构信息，先分配初始的空间 */
	...
	/* 为每一个groups中的指针分配内存空间，每一个就是一个FDFSGroupInfo结构 */
	...
	/* 为存放按照group_name排序好的groups信息分配空间 */
...
/* 释放每一个group已经分配的内存空间，以及最后存储指针的空间 */
...
static int tracker_mem_realloc_groups(FDFSGroups *pGroups, const bool bNeedSleep)
{
	...
	FDFSGroupInfo **new_groups;		/* 新分配groups对象数组的指针 */
	FDFSGroupInfo **new_sorted_groups;		/* 排序好的groups对象数组 */
	FDFSGroupInfo **ppGroup;
	FDFSGroupInfo **ppGroupEnd;
	int new_size;			/* 需要新分配的内存元素个数 */

	new_size = pGroups->alloc_size + TRACKER_MEM_ALLOC_ONCE;		/* 增加分配内存对象空间 */
	...
	{
		*ppGroup = (FDFSGroupInfo *)malloc(sizeof(FDFSGroupInfo));
		/* 如果分配内存空间失败，释放所有已分配空间 */
		if (*ppGroup == NULL)
		{
			/* 释放每一个group所占用的内存空间，以及最后存储指针的空间 */
...
/* 当前空间不够，重新多分配inc_count个数的storage空间 */
...
/* 将pTargetServer插入到排序好的sorted_servers数组中，count是sorted_servers原来的元素个数 */
		...
		FDFSGroupInfo *pTargetGroup)
{
	...
	/* 使用直接插入法将该group信息添加进已经排序好的groups中 */
		...
		const char *group_name)
{
	...
	/* 根据group_name从当前已经存在的groups中进行二分查找 */
	...
	const bool bNeedSleep, bool *bInserted)
{
	...
	{	
		...
		*bInserted = false;
		...
		/* 
		 * pGroups->groups + pGroups->count -1是最后的一个group地址
		 * 不减1即是将要新增的group地址，之前已经为此分配过内存
		 */
		...
		/* 如果配置文件中指定了特定组上传，并且已经指定的组名和新增组名相同 */
	...
	/* 将新增的group对象的指针放到pClientInfo对象中返回 */
	pClientInfo->pGroup = pGroup;
	return 0;
}

/* 根据storage_id获取一个处于active状态的storage server */
...
/* 根据ip地址获取处于active状态的storage server */
				...
				const char *ip_addr)
{
	...
	{
		FDFSStorageIdInfo *pStorageIdInfo;
		/* 在指定group_name的group中根据ip获取FDFSStorageIdInfo对象 */
...
int tracker_mem_delete_storage(FDFSGroupInfo *pGroup, const char *id)
{
	...
	/* 如果删除的这个storage是其他storage的源server，psync_src_server置空 */
		...
		const char *old_storage_ip, const char *new_storage_ip)
{
	...
	/* 根据新的ip地址获取storage server，如果已经存在，报错 */
	 ...
	 * 交换新旧storage对象，将旧的storage对象状态改为IP_CHANGED
	 */
		...
		TrackerRunningStatus *pStatus)
{
	...
	pInBuff = in_buff;
	/* 读取tracker向tracker发送的获取状态信息的返回报文 */
...
void tracker_calc_running_times(TrackerRunningStatus *pStatus)
{
	...
	/* 折算为TRACKER_SYNC_STATUS_FILE_INTERVAL的倍数 */
...
/*
 * file_index就指定了要传输的文件名，在全局变量中配置
 * 一个配置文件可能需要分成多个包来传输
 * offset控制每次需要读取的文件的起始部分，文件大小通过file_size返回
 */
	...
	const int file_index, int fd, int64_t *offset, int64_t *file_size)
{
	...
	/* 文件名索引，具体的文件名 存储在g_tracker_sys_filenames中*/
	*p++ = file_index;
	...
	*file_size = buff2long(in_buff);
	...
	/* 文件偏移量增加，下一次调用的时候传入该值，则继续传输文件的剩余部分 */
	*offset += write_bytes;
	return 0;
}

/* 从其他的tracker server获取一个配置文件，file_index为g_tracker_sys_filenames中文件名的索引 */
		...
		const int file_index)
{
	...
	offset = 0;
	file_size = 0;
	/* 通过socket从其他的tracker server获取该文件 */
	while (1)
	{
		/*
		 * file_index就指定了要传输的文件名，在全局变量中配置
...
static int tracker_mem_get_sys_files(ConnectionInfo *pTrackerServer)
{
	...
	/* 
	 * 依次从其他的tracker server获取一个配置文件
	 * file_index为g_tracker_sys_filenames中文件名的索引 
	 */
...
/* 对TrackerRunningStatus比较进行比较，先比较running_time，再比较restart_interval */
...
/* 
 * 如果当前是第一次加入tracker
 * 将新加入的storage server的的所有tracker server复制到g_tracker_servers中 
 */
...
/* 
 * 将新加入的storage中配置的tracker和本地的比较
 * 将新的tracker加入到本地的tracker信息中 
 */
static int tracker_mem_check_add_tracker_servers(FDFSStorageJoinBody *pJoinBody)
{
	...
	/* 遍历storage中配置的tracker和本地已经有的tracker相比，增加了多少新的tracker */
	add_count = 0;
...
/*
 * 向pJoinBody这个新加入的storage所配置的所有tracker发送获取状态信息的报文
 * 取运行时间最长，重启间隔时间最短的tracker的状态信息
 */
		...
		TrackerRunningStatus *pTrackerStatus)
{
	...
	/* 遍历storage server中配置的所有tracker server */
	...
	/* 对所有的tracker的TrackerRunningStatus排序，running_time越小，restart_interval越大，排在前面 */
...
/* 
 * 从其他tracker server(运行时间最长的一个)获取配置文件信息
 * 如果就是自身，什么都不做
 */
		 ...
		 TrackerRunningStatus *pRunningStatus)
{
	...
	{
		/* 用当前tracker的状态和其他所有tracker中运行时间最长的进行比较 */
		if (tracker_mem_cmp_tracker_running_status(pRunningStatus, \
							&trackerStatus) >= 0)
		{
			/* 如果大于0，说明自身的配置文件就是最新的，无需更新 */
			logDebug("file: "__FILE__", line: %d, " \
				"%s:%d running time: %d, restart interval: %d, "\
	...
	/* 从其他tracker获取到的groups配置文件来初始化内存中的groups信息 */
		...
		const bool bNeedSleep)
{
	...
	/* 如果之前没有更新完成过，并且当前groups数量为0，从其他tracker更新 */
	...
	/* 如果还没有其他tracker server，将其加入到g_tracker_servers.servers中 */
	...
	/* 如果该group的storage_port没有设置，设置为该storage的storage_port */
	...
	else
	{
			...
			pJoinBody->storage_port)
		{
			...
			/* 如果所有的group中的storage的storage_port都一致了，修改group的storage_port */
	...
	/* 如果之前的store_path_count，以新加入的storage_server的配置为准，分配相应空间 */
	...
	/* 如果之前处于offline或者recovery状态，改为online状态 */
 ...
 * 可以同步多个storage，briefServers数组的元素个数由server_count指定
 */
		...
		FDFSStorageBrief *briefServers, const int server_count)
{
	...
	do
	{
		...
		for (pServer=briefServers; pServer<pEnd; pServer++)
		{
			...
			{
				/* 如果原来storage正常，发送过来的报文为异常，需要修改状态 */
			...
			/* 如果目前还没有此storage的信息，而且状态是DELETED或者IP_CHANGED，不处理 */
			...
			/* 如果目前还没有此storage的信息，但是是正常状态，加入到内存中 */
			else
			{
				FDFSStorageDetail *pStorageServer;
				bool bInserted;
...
static void tracker_mem_find_store_server(FDFSGroupInfo *pGroup)
{
	...
	if (g_groups.store_server == FDFS_STORE_SERVER_FIRST_BY_PRI)
	{
		...
		/* 遍历当前group，选择upload_priority值最小的storage */
	...
	/* 否则直接选取active_servers数组中的第一个storage */
	else
	{
...
/* 向storage发送获取binlog_size的报文，binlog_size值在file_size变量中返回 */
	...
	ConnectionInfo *pStorageServer, int64_t *file_size)
{
	...
	/* 报文体应该是binlog_size，8字节数据，如果不是，说明报文有错 */
	if (in_bytes != sizeof(in_buff))
	{
		logError("file: "__FILE__", line: %d, " \
	...
	/* 报文中获取到的binlog_size在file_size变量中返回 */
	*file_size = buff2long(in_buff);
	return 0;
}
...
/* 将trunk_server的变更信息写入logs/trunk_server_change.log文件 */
		...
		FDFSStorageDetail *pLastTrunkServer)
{
	...
	/* 打开logs/trunk_server_change.log 文件 */
...
/* 设置pGroup的trunk_server为pNewTrunkServer，并将改变的信息写入文件中 */
		...
		FDFSStorageDetail *pNewTrunkServer)
{
	...
	/* 如果新的trunk_server_id和之前的trunk_server_id不一样 */
...
/* 
 * 设置pTrunkServer为pGroup的trunk server 
 * 如果save为true，则需要保存group信息到文件中
 */
	...
	FDFSStorageDetail *pTrunkServer, const bool save)
{
	...
	/* 如果之前的trunk server和要设置的不是同一个 */
	if (*(pGroup->last_trunk_server_id) != '\0' && 
	    strcmp(pTrunkServer->id, pGroup->last_trunk_server_id) != 0)
	{
		/* 向指定ip:port发送delete_binlog_marks的命令字节，不含报文体 */
		...
		const bool save)
{
	...
	/* 获取trunk的binlog_size，会向pStoreServer发送报文获取该值 */
	...
	/* 
	 * 发送报文获取所有处于active状态的storage server的trunk_binlog_size
	 * 选取binlog_size最大的一个服务器
	 */
	ppServerEnd = pGroup->active_servers + pGroup->active_count;
	for (ppServer=pGroup->active_servers; ppServer<ppServerEnd; ppServer++)
	{
			...
			FDFSStorageDetail *pTargetServer) 
{
	...
	/* 重新设置上传文件时选择当前group中的哪一个storage */
	tracker_mem_find_store_server(pGroup);
...
/* 
 * 将storage server激活，如果之前没有，就加入到相应group中
 * 通常是接收到storage server发送过来的心跳包或其他报文后的操作
 */
			...
			FDFSStorageDetail *pTargetServer) 
{
	...
	*/
	...
	/* 将此storage server的状态改为Active */
	pTargetServer->status = FDFS_STORAGE_STATUS_ACTIVE;

	/* 
	 * 这里将采用两次检查的方式
	 * 先检查一次，满足条件后加锁，之后再检查一次，提高效率，避免每次都加锁
	 */
	/* 检查该group中此storage是否已经存在于active_servers的数组中，如果存在，直接返回 */
	...
	/* 检查该group中此storage是否已经存在于active_servers的数组中 */
	ppStorageServer = (FDFSStorageDetail **)bsearch(&pTargetServer, \
			pGroup->active_servers, \
			pGroup->active_count,   \
			sizeof(FDFSStorageDetail *), \
	...
	{
		/* 
		 * 将pTargetServer插入到排序好的sorted_servers数组中
		 * count是sorted_servers原来的元素个数 
		 */
...
/* 根据file_name获取应该去哪台storage server下载或更新 */
	...
	int *server_count)
{
	...
	*ppGroup = tracker_mem_get_group(group_name);
	...
	else //file generated by version >= v1.12
	{
		...
		/* 从filename中解码获取ip,时间戳以及file_size */
	...
	*/
	...
	{
		/* 
		 * 如果设置为下载文件选用之前上传的storage的方式 
		 * 之前从文件名中解析出了源storage的id或ip
		 */
		...
		*/
		...
		do
		{
			...
			if (bNormalFile)
			{
				current_time = g_current_time;
				/* 
				 * 当前时间减去文件上传的时间超过了最大延迟时间 
				 * 或者此storage的最近一次同步时间在此文件上传时间之后
				 * 或者前两个同时满足
				 * 或者没有指定ip地址并且采用循环的方式来上传文件
				 * 直接跳出do-while，read_server_index即为要下载文件的storage
				 */
				...
				/* 如果没有ip信息，设置下载的storage为上传优先级最高的storage */
			...
			/* 如果下载storage和之前上传文件的storage是同一个 */
		     	if (cmp_res == 0)
		      	{
#ifdef WITH_HTTPD
				if (download_type == FDFS_DOWNLOAD_TYPE_TCP)
			...
			/* 
			 * 如果采用循环的方式但是有可能还没有及时同步到另外一台storage 
			 * 改为从当初上传文件的storage下载
			 */
			if (g_groups.download_server == \
			...
			/* 如果这个group上传文件时没有选择循环的方式，选取优先级高的storage */
	...
	else //TRACKER_PROTO_CMD_SERVICE_QUERY_FETCH_ALL
	{
		...
		{
			/* 
			 * 遍历这个组中的每一个storage_server
			 * 通过时间戳选择在文件上传过之后全部同步过一次的storage 
			 * 加能下载到该文件的storage全部返回
			 */
			current_time = g_current_time;
...
int tracker_mem_check_alive(void *arg)
{
	...
	/* 
	 * 遍历所有active_servers
	 * 查看当前时间和最后一次接收到心跳包的时间是否超过指定的间隔 
	 * 如果超过了，说明此storage已经下线了
	 */
	ppGroupEnd = g_groups.groups + g_groups.count;
	for (ppGroup=g_groups.groups; ppGroup<ppGroupEnd; ppGroup++)
	{
		...
		/* 将所有下线的storage移除出active_servers数组，并重新选择上传文件的storage */
	...
	/* 
	 * tracker_leader并且使用trunk_file才进行的操作 
	 * 如果trunk_server下线了，需要重新指定trunk_server
	 */
	for (ppGroup=g_groups.groups; ppGroup<ppGroupEnd; ppGroup++)
	{
		if ((*ppGroup)->pTrunkServer != NULL)
