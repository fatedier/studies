...
#define TRACKER_PROTO_CMD_STORAGE_BEAT              83  /* storage heart beat(心跳协议) */
...
#define FDFS_PROTO_PKG_LEN_SIZE		8		/* 报文体长度字符串的字节数 */
#define FDFS_PROTO_CMD_SIZE		1
#define FDFS_PROTO_IP_PORT_SIZE		(IP_ADDRESS_SIZE + 6)	/* ip地址加端口号的长度 */
...
typedef struct
{
	char pkg_len[FDFS_PROTO_PKG_LEN_SIZE];  /* body length, not including header (报文体长度) */
	char cmd;    		/* command code (指令码) */
	char status; 		/* status code for response(应答状态) */
} TrackerHeader;	/* tracker报文头 */
...
} TrackerStorageJoinBody;		/* storage请求加入的报文体 */

typedef struct
{
	char src_id[FDFS_STORAGE_ID_MAX_SIZE];  //src storage id
} TrackerStorageJoinBodyResp;	/* storage加入请求的应答报文体 */
...
} TrackerStorageSyncReqBody;	/* storage的同步信息返回报文体 */

typedef struct
{
	char sz_total_mb[8];		/* 总使用量 */
	char sz_free_mb[8];		/* 空闲空间 */
} TrackerStatReportReqBody;	/* storage汇报disk使用情况的报文体 */
...
**/
/* 建立到pTrackerServer的连接，返回ConnectionInfo指针，失败返回NULL */
...
/* 
 * 向所有tracker server发送报文获取配置参数的参数信息存放在iniContext中
 * 直到成功从一个tracker上获取到相关配置信息
 */
