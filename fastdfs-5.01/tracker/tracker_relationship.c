...
/*
 * 向tracker_leader发送ping_leader报文 
 * 发送报文:无
...
static int fdfs_ping_leader(ConnectionInfo *pTrackerServer)
{
					...
					FDFS_STORAGE_ID_MAX_SIZE)
	{
		...
		/* 设置从报文中解析出的每一个group的最近使用的trunk_server_id */
...
/* 从tracker server中选择一个leader，该tracker的信息通过pTrackerStatus返回 */
static int relationship_get_tracker_leader(TrackerRunningStatus *pTrackerStatus)
{
	...
	/* 对tracker的状态信息进行排序，依次比较if_leader, running_time, restart_interval, ip, port */
...
/* 通知所有的tracker server，tracker leader变更 */
...
static int relationship_select_leader()
{
	...
	/* 从tracker server中选择一个适合的leader，该tracker的信息通过pTrackerStatus返回 */
	if ((result=relationship_get_tracker_leader(&trackerStatus)) != 0)
	{
		return result;
	}

	/* 如果该tracker就是自己，通知所有的tracker server，tracker leader变更 */
		...
		is_local_host_ip(trackerStatus.pTrackerServer->ip_addr))
	{
		...
		/* 自身变为leader了，需要查找所有group的trunk_server */
		tracker_mem_find_trunk_servers();
	}
	/* 否则设置tracker_leader索引 */
	else
	{
...
static int relationship_ping_leader()
{
	...
	/* 
	 * 向tracker_leader发送ping_leader报文
	 * 根据返回报文设置每一个group的最近使用的trunk_server_id 
	 */
...
#define MAX_SLEEP_SECONDS  10
	...
	while (g_continue_flag)
	{
		...
		/* 如果缓存的之前的tracker_server信息不为空，释放掉 */
