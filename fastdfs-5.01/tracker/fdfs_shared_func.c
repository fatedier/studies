...
/* sorted by group name and storage IP(存放根据group name和ip排序的storage server信息) */
FDFSStorageIdInfo *g_storage_ids_by_ip = NULL;
/* sorted by storage ID(存放根据server id排序的storage server信息) */
FDFSStorageIdInfo **g_storage_ids_by_id = NULL;
int g_storage_id_count = 0;	/* storage ids file中配置数量 */
		...
		FDFSStorageReservedSpace *pStorageReservedSpace)
{
	...
	/* 获取配置文件中reserved_storage_space字段值 */
	...
	pStorageReservedSpace->rs.mb = storage_reserved / FDFS_ONE_MB;	/* 将存储容量从字节转换为MB */
	return 0;
}
...
/* 检验server id值是否正确，是否是数字，是否在指定范围内 */
...
/* 在所有storage中根据id获取FDFSStorageIdInfo对象 */
...
int fdfs_load_storage_ids(char *content, const char *pStorageIdsFilename)
{
	...
	do
	{
		...
		for (i=0; i<line_count; i++)
		{
			...
			/* 获取storage server的id，group_name指针会在id结束的地方加'\0' */
			id = line;
			group_name = line;
			/* 取数据，直到遇到空格，tab或字符串结束符 */
	...
	/* 
	 * 把g_storage_ids_by_ip中的数据复制到g_storage_ids_by_id中
	 * 前者是根据group_name和ip排序，后者根据server_id排序
	 */
...
/* 从文件中获取storage server组名，id及其对应的ip地址 */
