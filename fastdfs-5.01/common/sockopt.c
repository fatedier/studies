...
int tcpsenddata_nb(int sock, void* data, const int size, const int timeout)
{
	...
	left_bytes = size;
	/* 
	 * 向sock发送size长度的数据，如果中间有错
	 * 利用select/poll等待该sockfd可写，继续写入剩下的内容 
	 */
	while (left_bytes > 0)
	{
		write_bytes = send(sock, p, left_bytes, 0);
...
/* 
 * 使用参数中传入的getname函数，根据socket描述符获取对应ip地址 
 * in_addr_t类型的地址通过函数返回
 * 字符串类型的地址放在传入的buff字符串中
 */
...
/* 
 * 根据域名获取ip地址，buff如果不传NULL，则返回时为ip地址的字符串表示
 * name可以传域名或ip地址，返回值为in_addr_t类型
 */
...
/* 
 * 通过建立连接的sock，接收文件写入filename
 * 文件大小为file_bytes 
 * fsync_after_written_bytes指定写入多少字节后，使用fsync强制刷新到磁盘
 * true_file_bytes为实际获取到的文件的大小
 */
		...
		int64_t *true_file_bytes)
{
	...
	int flags;
	int count;
	tcprecvdata_exfunc recv_func;	/* 接收一次报文的函数 */
...
*/
	...
	/* 
	 * l_onoff = 0,等于内核缺省情况，close调用会立即返回给调用者
	 * 如果可能将会传输任何未发送的数据
	 */
	linger.l_onoff = 0;
	linger.l_linger = 0;
...
#if defined(OS_LINUX) || defined(OS_FREEBSD)
/* 如果是OS_LINUX或OS_FREEBSD系统，获取所有本地ip地址存放在ip_addrs[]中 */
...
#else
/* 如果不是OS_LINUX或OS_FREEBSD系统，获取所有本地ip地址存放在ip_addrs[]中 */
int getlocaladdrs(char ip_addrs[][IP_ADDRESS_SIZE], \
	const int max_count, int *count)
{
	int s;
