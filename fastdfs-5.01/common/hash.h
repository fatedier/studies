...
typedef struct tagHashArray
{
	HashData **buckets;
	HashFunc hash_func;
	int item_count;		/* 所有哈希桶中的hash_data的总数 */
	unsigned int *capacity;
	double load_factor;	/* 装填因子 */
	int64_t max_bytes;	/* 最大占用字节 */
	int64_t bytes_used;	/* 已经使用字节 */
	bool is_malloc_capacity;	/* 容量不够时新增空间是选取比当前空间大的下一个素数，还是简单加1 */
	bool is_malloc_value;		/* 如果key值重复，是否需要另外分配空间，false则覆盖原来的值 */
	unsigned int lock_count;	/* 锁的数量 */
