...
/* 在哈希桶中查找指定key值的对象是否存在，若存在，返回相应hash_data */
...
/* 根据key值在哈希表中查找value，返回整个hash_data对象 */
...
/* 根据key值在哈希表中查找value，返回void *类型的value值 */
		...
		void *value, const int value_len, const bool needLock)
{
	...
	if (needLock)
	{
		/* 对需要存储的哈希桶加锁，pBucket - pHash->buckets相当于数组下标 */
		HASH_LOCK(pHash, ppBucket - pHash->buckets)
	}

	/* 获取哈希桶中第一个hash_data的地址 */
	hash_data = *ppBucket;
	/* 如果哈希桶中已经有值，检查key值是否已经存在 */
	while (hash_data != NULL)
	{
		if (key_len == hash_data->key_len && \
			memcmp(key, hash_data->key, key_len) == 0)
	...
	if (hash_data != NULL) //exists
	{
		...
		{
			/* 如果malloc_value_size空间足够，直接覆盖原来的值 */
	...
	/* 将构造的hash_data插入到哈希桶的相应位置中，对应哈希桶的头节点 */
	if (needLock)
	{
		HASH_LOCK(pHash, ppBucket - pHash->buckets)
