...
/* 读取文件中指定部分内容放到buff字符串中，offset开始长度为size的内容 */
...
/* 将buff字符串的前file_size个内容写入到filename文件中 */
...
/* 从配置文件结构体中获取log_level字段，并设置当前日志级别 */
...
{
	printf("wcl: current log level: %s\n", pLogLevel);
	if (pLogLevel != NULL)
	{
...
{
	int count;				/* allow_hosts对应值数组元素个数(配置文件中获取到的) */
	IniItem *pItem;			/* 用于遍历allow_host值数组的指针 */
	IniItem *pItemStart;		/* allow_hosts对应值数组的首个元素 */
	IniItem *pItemEnd;		/* allow_hosts对应值数组的末尾元素 */
	char *pItemValue;		/* 用来临时存储allow_hosts值的字符串 */
	char *pStart;				/* 值字符串"["对应位置 */
	char *pEnd;				/* 值字符串"]"对应位置 */
	char *p;
	char *pTail;
	int alloc_count;			/* 要分配内存的元素个数 */
	int nHeadLen;
	int i;
	in_addr_t addr;
	char hostname[256];		/* ip地址字符串 */

	/* 获取配置文件中allow_hosts对应的值，可以有多个 */
	if ((pItemStart=iniGetValuesEx(NULL, "allow_hosts", \
		pIniContext, &count)) == NULL)
	{
		/* 如果 没有配置allow_hosts的值，默认匹配任何ip地址 */
	...
	*allow_ip_addrs = (in_addr_t *)malloc(sizeof(in_addr_t) * alloc_count);
	...
	for (pItem=pItemStart; pItem<pItemEnd; pItem++)
	{
		...
		/* 解析类似于allow_hosts=10.0.1.[1-15,20] 或allow_hosts=host[01-08,20-25].domain.com的格式 */
		nHeadLen = pStart - pItem->value;	/* "["到字符串首位的长度 */
		...
		while (p <= pEnd)
		{
			...
			int nStart;			/* 第一个数字值 */
			int nEnd;				/* 第二个数字值 */
			int nNumLen1;		/* 第一个数字对应字符串的长度 */
			int nNumLen2;		/* 第二个数字对应字符串的长度 */
			char end_ch1;		/* 第一个数字后的字符 */
			char end_ch2;		/* 第二个数字后的字符 */
			char szFormat[16];
...
int parse_bytes(char *pStr, const int default_unit_bytes, int64_t *bytes)
{
	...
	pReservedEnd = NULL;
	/* 将字符串转换为64位整型，剩下非数字部分放在pReservedEnd中 */
...
/* 设置定时器，延迟first_remain_seconds后，每间隔interval秒，调用sighandler函数 */
		...
		void (*sighandler)(int))
{
	...
	value.it_interval.tv_sec = interval;	/* 指定间隔时间 */
	value.it_value.tv_sec = first_remain_seconds;	/* 指定一次执行的延迟时间 */
	/* 设置定时器，以系统真实时间来计算 */
