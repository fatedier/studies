...
/* 比较两个key名称是否相等，用于bsearch二分查找等函数中 */
...
int iniLoadFromFile(const char *szFilename, IniContext *pContext)
{
	...
	/* 如果是网络路径 */
	if (strncasecmp(szFilename, "http://", 7) == 0)	/* 忽略大小写比较前n个字符串 */
	{
	...
	{
		if (*szFilename == '/')	/* 如果是绝对路径 */
		{
			pLast = strrchr(szFilename, '/');		/* 返回最后一个'/'的位置指针 */
			len = pLast - szFilename;
			if (len >= sizeof(pContext->config_path))		/* 超出路径名最大长度 */
			{
				logError("file: "__FILE__", line: %d, "\
			...
			memcpy(pContext->config_path, szFilename, len);	/* 保存配置文件的basename */
		...
		else
		{
			...
			if (getcwd(pContext->config_path, sizeof( 		/* 获取当前工作目录 */
...
/* 根据配置文件初始化对应哈希表，加载到内存中 */
static int iniDoLoadFromFile(const char *szFilename, \
		IniContext *pContext)
{
	char *content;		/* 配置文件内容 */
	int result;
	int http_status;		/* http返回状态码 */
	int content_len;		/* 网络配置文件内容大小 */
	int64_t file_size;		/* 本地配置文件大小 */
	char error_info[512];	/* 网络传输错误信息 */
		...
		IniContext *pContext, int *nTargetCount)
{
	...
	*nTargetCount = 1;
	...
	/* 返回查找到的匹配的数组，元素个数在nTargetCount中 */
	return pItemStart;
}
