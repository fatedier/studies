...
type Data struct {
	...
	ClusterID uint64
	Databases []DatabaseInfo		// 所有数据库信息
	Users     []UserInfo			// 所有用户信息

	MaxShardGroupID uint64			// 最大 shardGroupID
	MaxShardID      uint64			// 最大 shardID
...
func (data *Data) CreateShardGroup(database, policy string, timestamp time.Time) error {
	...
	sgi := ShardGroupInfo{}
	sgi.ID = data.MaxShardGroupID	// shardGroup 的ID 递增
	sgi.StartTime = timestamp.Truncate(rpi.ShardGroupDuration).UTC()	// 存储 Points 的起始时间
	sgi.EndTime = sgi.StartTime.Add(rpi.ShardGroupDuration).UTC()		// 存储 Points 的结束时间
	if sgi.EndTime.After(time.Unix(0, models.MaxNanoTime)) {
		// Shard group range is [start, end) so add one to the max time.
		// 这里因为前面 end 都是开区间，达到最大值的话，后面就没有了，所以这里需要用闭区间
...
type DatabaseInfo struct {
	Name                   string					// 数据库名
	DefaultRetentionPolicy string					// 默认存储策略
	RetentionPolicies      []RetentionPolicyInfo	// 存储策略
	ContinuousQueries      []ContinuousQueryInfo	// CQ
}
...
type RetentionPolicyInfo struct {
	Name               string				// 存储策略名
	ReplicaN           int					// 副本数
	Duration           time.Duration		// 数据实际的保存时间，默认为永久保存
	ShardGroupDuration time.Duration		// 每一个 shard 中数据保存的时间范围，默认为 7 天
	ShardGroups        []ShardGroupInfo		// 已经被创建的 shardGroup 信息
...
// 通过 rp 中需要保存数据的时间，决定单个 shard 中保存数据的时间范围
...
type ShardGroupInfo struct {
	ID          uint64
	StartTime   time.Time		// 存储数据的开始时间，闭区间
	EndTime     time.Time		// 存储数据的结束时间，开区间
	DeletedAt   time.Time		// 如果这个时间被设置，系统可以安全的删除所有相关联的 shard
	Shards      []ShardInfo		// group 中 shard 的信息
	TruncatedAt time.Time
}

// ShardGroupInfos implements sort.Interface on []ShardGroupInfo, based
// on the StartTime field.
// 用于对 shardGroup 进行排序，会根据 shardGroup 所保存的时间段来排序，越早的时间段位置越前
...
// 根据 Point 的 hash 值返回在这个 shardGroup 中的一个 shard 对象
