...
// 返回当前生成器中文件的级别
// 0 表示刚从 cache 中刷新数据到磁盘上的 tsm 文件
// 2, 3 表示合并了多个下一级的 tsm 文件
// 4 表示其他情况
...
func (c *DefaultPlanner) PlanLevel(level int) []CompactionGroup {
	...
	// 按照级别将文件生成器分组，相邻的并且 level 相同的生成器会被放入同一个组中
	var currentGen tsmGenerations
	var groups []tsmGenerations
	// generations 中的数据在之前已经按照 id 进行过排序了
	...
	// minGenerations 3 -> 4
	// minGenerations 4 -> 4
	// 每一个级别所需的文件数不同，因为级别越高，合并对cpu的影响越大，所以有足够的数据时才进行
...
// 从多个小的 tsm 文件中读取内容，合并到新的 tsm 文件
func (c *Compactor) compact(fast bool, tsmFiles []string) ([]string, error) {
	...
	// 合并后的文件会被写入这批 tsm 文件中 generateId 最大的生成器
	// 且 level 级别 + 1
	...
	// 将 tsm 文件数据迭代器中的内容写入新的文件
	// 会被写到 maxGeneration 的 maxSequence + 1 的序号的文件中
...
// tsm key 的数据迭代器，从底层多个 tsm 文件中获取数据，依次返回排好序且去重了的数据
type tsmKeyIterator struct {
	...
	// 在压缩合并时这个迭代器是否使用快速模块，如果为 true，多个 block 会直接返回，不会进行组合
	// 否则，将会反序列化 blocks 中的内容，并且去重，之后合并到新的文件中
	fast bool
