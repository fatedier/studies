...
type FileStore struct {
	...
	dir               string		// shard 所在目录

	files []TSMFile					// 此 shard 下所有 tsm 文件对象
...
// 是否包含 min-max 的时间范围，有交集就返回 true
...
type FileStoreStatistics struct {
	DiskBytes int64		// 该 shard 下所有 tsm 文件大小
	FileCount int64		// 该 shard 下的文件数
}
...
// 遍历函数，对每一个 key 执行 fn 函数，如果一个 key 在多个 tsm 文件中，对每一个文件中的该 key 都执行此函数
...
// DeleteRange removes the values for keys between min and max.
// 移除指定的那些 key 在 min 到 max 时间范围内的数据
...
func (f *FileStore) Open() error {
	...
	// 从此 shard 的目录下找到以 .tmp 结尾的目录的最大的 ID
...
// 获取指定 key， 指定时间的数据所在的位置，location 中包括了所在的 tsmFile 以及所有 block 的索引信息
func (f *FileStore) locations(key string, t int64, ascending bool) []*location {
	...
	for _, fd := range filesSnapshot {
		...
		// the given key.
		// 获取所有该 tsm 文件中包含此 key 的 block的索引数据
...
// 解析 tsm 文件名，得到 generation 和 sequence ID
...
type KeyCursor struct {
	...
	// been overwritten.
	// 当前遍历中的 block
	// 大多数时间只有一个，但是有时会有多个，比如数据没有按照时间顺序插入，或者之后有覆盖原先的时间戳写入的数据，并且 tsm 文件还没有来得及合并
	current []*location
	buf     []Value
	...
	// seeks 中的 block 数据的时间范围是否有重合的部分
	// 如果为 true，则在后续的一些处理中需要特殊处理，从多个 block 读取数据并且合并
	duplicates bool
}

// 单个 block 的位置信息
type location struct {
	r     TSMFile			// 所在 tsm 文件对象
	entry IndexEntry		// 一个 block 在内存中的索引信息

	readMin, readMax int64
}

// 返回该 block 中是否已经全部读取完成
...
// 返回当前的所有 block 的数据的时间段中是否有重合的部分
// 这个数据计算一次后会存在 "duplicates" 字段中
// 如果没有重合的部分，之后的查询操作就不需要合并，只需要从一个 block 中获取数据直接返回，速度会快很多
...
func (c *KeyCursor) seekAscending(t int64) {
	for i, e := range c.seeks {
		if t < e.entry.MinTime || e.entry.Contains(t) {
			...
			// 如果不需要去重，直接返回，否则将包含时间 t 的 block 都加入到 current 数组中
			if !c.duplicates {
				return
			}
		}
...
// 将 cursor 移动到下一条数据，如果缓存的数据读完了，会调用 ReadBlock 读取
func (c *KeyCursor) Next() {
	if len(c.current) == 0 {
		return
	}
	// Do we still have unread values in the current block
	// 当前 block 中是否仍然有未读的数据，如果有的话直接返回
	// read 返回的 bool 值表示该 block 中的数据是否已经全部读完
	if !c.current[0].read() {
		return
	}
	// 如果已经读取完，获取下一个 block 的数据
