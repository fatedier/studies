...
type DatabaseIndex struct {
	...
	// 所有 Series 对象，SeriesKey = measurement + tags
...
type IndexStatistics struct {
	NumSeries       int64	// 所有 Series 的数量
	NumMeasurements int64	// 所有 Measurement 的数量
}
...
// 返回此数据库中指定 name 的 Measurement 信息
...
// 返回当前数据库中的 measurement 数量以及 series 数量
...
// 根据 influxql 的表达式过滤出符合条件的 Measurements
// 第二个 bool 值如果为 false 表示 influxql 表达式中并没有 measurement 相关的条件
...
func (d *DatabaseIndex) measurementsByExpr(expr influxql.Expr) (Measurements, bool, error) {
	...
	case *influxql.BinaryExpr:
		...
		case influxql.EQ, influxql.NEQ, influxql.EQREGEX, influxql.NEQREGEX:
			...
			if tag.Val == "_name" {
				// 过滤 measurement，例如 _name = 'cpu_load'，或者右边是一个正则表达式，会返回所有符合要求的 measurement
				return d.measurementsByNameFilter(tf.Op, tf.Value, tf.Regex), true, nil
			} else if influxql.IsSystemName(tag.Val) {
			// 如果表达式的 key 不是 measurement，并且是系统内部的其他名字
				return nil, false, nil
			}
...
// 返回排序好的符合过滤条件的所有 measurement，直接判断 measurement 是否相等，或者根据正则表达式来判断
...
// 通过 measurement 中所有存在的 tagk 和 tagv 来过滤，支持正则表达式
func (d *DatabaseIndex) measurementsByTagFilters(filters []*TagFilter) Measurements {
	...
	for _, m := range d.measurements {
		...
		for _, f := range filters {
			m.mu.RLock()
			// 获取 measurement 中 tagk = f.Key 的所有 tagv
...
// 检查 group by 的中的 fieldName 在当前 Measurement 中是否存在
...
func (m *Measurement) AddSeries(s *Series) bool {
	...
	// 将此 series 根据 tags 加入到所有相对应的 双层 map 中，为了便于之后根据 tags 来快速检索到要查询的 seriesID
...
// 根据 influxql 表达式过滤出符合要求的所有 seriesID
...
type TagFilter struct {
	Op    influxql.Token	// 运算符
	Key   string			// tagk
	Value string			// tagv
	Regex *regexp.Regexp	// 正则表达式匹配
}
