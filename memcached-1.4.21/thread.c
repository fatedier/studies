...
#define ITEMS_PER_ALLOC 64	/* 一次性分配ITEM的个数 */

/* An item in the connection queue. */
typedef struct conn_queue_item CQ_ITEM;
struct conn_queue_item {		/* 连接队列中的一个连接对象 */
    int               sfd;	/* 客户端文件描述符 */
    enum conn_states  init_state;	/* 连接状态 */
    int               event_flags;
    int               read_buffer_size;
    enum network_transport     transport;	/* 连接类型 */
    CQ_ITEM          *next;
};

/* A connection queue. */
typedef struct conn_queue CQ;
struct conn_queue {	/* 连接队列 */
    CQ_ITEM *head;
    CQ_ITEM *tail;
...
static CQ_ITEM *cqi_freelist;	/* CQ_ITEM的对象池 */
static pthread_mutex_t cqi_freelist_lock;

static pthread_mutex_t *item_locks;			/* hash表元素锁数组 */
/* size of the item lock hash table */
static uint32_t item_lock_count;			/* hash表元素锁的个数 */
static unsigned int item_lock_hashpower;	/* hash表元素锁的个数的指数 */
#define hashsize(n) ((unsigned long int)1<<(n))
#define hashmask(n) (hashsize(n)-1)
/* this lock is temporarily engaged during a hash table expansion */
static pthread_mutex_t item_global_lock;	/* 仅在hash表扩展的时候有用 */
/* thread-specific variable for deeply finding the item lock type */
static pthread_key_t item_lock_type_key;

static LIBEVENT_DISPATCHER_THREAD dispatcher_thread;		/* 事件分发线程结构信息 */
 ...
 */
static int init_count = 0;		/* 初始化完成的工作线程数 */
...
static void wait_for_thread_registration(int nthreads) {
    /* 等待init_count == nthreads，每次有工作线程初始化完成时会发送信号唤醒 */
 ...
 * 从cqi_freelist中获取一个可用的CQ_ITEM对象，没有就继续分配内存空间
 */
...
static void setup_thread(LIBEVENT_THREAD *me) {
    ...
    /* 监听notify_receive_fd可读事件，回调函数为thread_libevent_process */
...
static void thread_libevent_process(int fd, short which, void *arg) {
    ...
    if (NULL != item) {
	 /* 
	  * 新建一个客户端连接对象
	  * 监听此客户端的读事件，调用event_handler函数 进行处理
	  */
        conn *c = conn_new(item->sfd, item->init_state, item->event_flags,
                           item->read_buffer_size, item->transport, me->base);
        if (c == NULL) {
	     /* 创建客户端连接对象失败，如果是udp，退出程序，否则断开连接 */
...
static int last_thread = -1;		/* 最后一次分配任务的工作线程 */
...
void thread_init(int nthreads, struct event_base *main_base) {
    ...
    /* 
     * 每一个工作线程通过管道和dispatcher_thread进行通信 
     * 当获取到一个连接后通过管道分发到相应工作线程进行处理
     */
