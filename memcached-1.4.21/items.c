...
/* LRU队列，数组的每一个与slabclass_id对应 */
static item *heads[LARGEST_ID];	/* LRU head buckets，双向链表每次插入放最前面 */
static item *tails[LARGEST_ID];		/* LRU tail buckets，双向链表，记录了LRU队列的最后一个元素 */
static crawler crawlers[LARGEST_ID];
static itemstats_t itemstats[LARGEST_ID];
static unsigned int sizes[LARGEST_ID];	/* 每一个LRU队列中item的个数 */
...
item *do_item_alloc(char *key, const size_t nkey, const int flags,
    ...
    item *it = NULL;
    char suffix[40];
    /* 返回item占用的总大小，并且拼suffix字符串，格式为flags nbytes\r\n */
    ...
    for (; tries > 0 && search != NULL; tries--, search=next_it) {
	 ...
	 /* 根据key计算出hash值 */
        ...
        if ((search->exptime != 0 && search->exptime < current_time)
            ...
            it = search;
	     /* 调整old_item和新item之间的差值 */
            slabs_adjust_mem_requested(it->slabs_clsid, ITEM_ntotal(it), ntotal);
            do_item_unlink_nolock(it, hv);
            /* Initialize the item block: */
            it->slabs_clsid = 0;
        } else if ((it = slabs_alloc(ntotal, id)) == NULL) {
            /* 如果没有找到，则首先分配一个新的item */
	     /* 内存耗尽的处理 */
            tried_alloc = 1;
    ...
    it->nkey = nkey;
    it->nbytes = nbytes;
    memcpy(ITEM_key(it), key, nkey);	/* 存放key */
    it->exptime = exptime;
...
int do_item_link(item *it, const uint32_t hv) {
    ...
    it->time = current_time;	/* 设置最近访问时间为当前时间 */
...
/*
 * 用new_item替代旧的item
 * 先调用do_item_unlink()解除原有it的连接，再调用do_item_link()连接到新的new_it 
 */
...
char *do_item_cachedump(const unsigned int slabs_clsid, const unsigned int limit, unsigned int *bytes) {
    ...
    while (it != NULL && (limit == 0 || shown < limit)) {
	 ...
	 /* 每一行是一个item的信息，包括key，value大小，过期时间 */
