...
typedef struct {
    unsigned int size;      /* sizes of items(这组class的slab中每个元素的size) */
    unsigned int perslab;   /* how many items per slab(每一个slab中可以存储多少个元素) */

    void *slots;           /* list of item ptrs(此class中可用items链表) */
    unsigned int sl_curr;   /* total free items in list(此class中总计可用items个数) */

    unsigned int slabs;     /* how many slabs were allocated for this class(此class中已分配空间的slab的个数 */

    void **slab_list;       /* array of slab pointers(所有slab的指针数组) */
    unsigned int list_size; /* size of prev array(slab的总数) */

    unsigned int killing;  /* index+1 of dying slab, or zero if none */
    size_t requested; /* The number of requested bytes(请求使用的空间大小) */
} slabclass_t;	/* slab信息结构 */

static slabclass_t slabclass[MAX_NUMBER_OF_SLAB_CLASSES];	/* slab相关信息的数组 */
static size_t mem_limit = 0;		/* 最大内存占用空间 */
static size_t mem_malloced = 0;	/* 已分配的内存空间大小 */
static int power_largest;			/* slab最大下标值 */

static void *mem_base = NULL;		/* 内存空间块首地址的指针 */
static void *mem_current = NULL;	/* 指向内存空间中当前可用块的指针 */
static size_t mem_avail = 0;		/* 内存空间当前可用大小 */
...
void slabs_init(const size_t limit, const double factor, const bool prealloc) {
    ...
    unsigned int size = sizeof(item) + settings.chunk_size;	/* 单个元素最小占用空间 */

    mem_limit = limit;		/* 最大内存占用空间 */
    ...
    while (++i < POWER_LARGEST && size <= settings.item_size_max / factor) {
	 ...
	 /* 8字节对齐 */
        ...
        slabclass[i].size = size;	/* 该slab中每个元素的大小 */
        slabclass[i].perslab = settings.item_size_max / slabclass[i].size;	/* 每个slab中的元素个数 */
	 /* 按照指定的factor倍数递增 */
        size *= factor;
...
/* 将slabclass[id]的slab数翻一倍 */
static int grow_slab_list (const unsigned int id) {
    slabclass_t *p = &slabclass[id];
    if (p->slabs == p->list_size) {
	 /* 同一class中slab个数翻一倍或者初始化为16 */
...
/* 释放slabclass[id]中ptr所指向的slab中的每一个元素，初始化相应值 */
...
static int do_slabs_newslab(const unsigned int id) {
    slabclass_t *p = &slabclass[id];
    /* 如果允许再分配，大小为item_size_max */
    int len = settings.slab_reassign ? settings.item_size_max
        : p->size * p->perslab;
    char *ptr;

    /* 如果内存分配没有超出上限，将slabclass[id]的slab数翻一倍，分配长度为size的内存空间 */
...
static void *do_slabs_alloc(const size_t size, unsigned int id) {
    ...
    /* 如果空间不够，为指定id的slabclass分配一个新的slab空间 */
...
static void do_slabs_free(void *ptr, const size_t size, unsigned int id) {
    ...
    it = (item *)ptr;
    it->it_flags |= ITEM_SLABBED;	/* 标志修改为归还内存到slab中 */
    it->prev = 0;
    it->next = p->slots;
 ...
 * slab平衡线程，负责选定需要移动的src_slab和dest_slab，通过信号量使得rebalance线程开始工作
 */
 ...
 * slab重新分配线程，接收到信号量后重新分配src的空间给dest
 */
