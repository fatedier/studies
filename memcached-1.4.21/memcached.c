...
/** exported globals **/
struct stats stats;
struct settings settings;	/* 各种参数设置 */
 ...
 * 连接结构的初始化工作，先全部置为NULL，具体内容空间用到的时候再分配 */
 */
static void conn_init(void) {
...
conn *conn_new(const int sfd, enum conn_states init_state,
    ...
    c->transport = transport;	/* 网络连接类型 */
    c->protocol = settings.binding_protocol;	/* 协议类型 */
    ...
    /* 等待响应此连接的读事件，调用event_handler函数进行处理 */
...
/* 检查统计信息字符串的空间是否足够，不足的话按2倍来扩展 */
static bool grow_stats_buf(conn *c, size_t needed) {
    size_t nsize = c->stats.size;	/* 当前统计信息字符串的长度 */
    size_t available = nsize - c->stats.offset;	/* 可使用的长度 */
    bool rv = true;
 ...
 *   }
 * 解析一条命令，按照空格分割
 * 每一段存放在tokens数组中，为字符串value和长度length
 */
...
/* 添加一条统计信息到统计字符串中，fmt为格式化字符串，后面是参数 */
...
static void drive_machine(conn *c) {
    ...
    while (!stop) {
	 ...
	 /* 读取数据 */
        case conn_read:
	     /* 读取数据，返回值为处理的结果，读取到的内容放在c->rbuf中 */
...
int main (int argc, char **argv) {
    ...
    /* 连接结构的初始化工作，先分配指针空间，具体内容空间用到的时候再分配 */
    conn_init();
    /* 初始化slab相关，分配空间等 */
    ...
    /* 
     * 启动hash表扩展线程，会阻塞在maintenance_cond条件变量上
     * 如果插入的时候超过 指定个数，会扩展旧的hash表
     */
