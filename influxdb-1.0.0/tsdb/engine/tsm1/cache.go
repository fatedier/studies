...
type entry struct {
	mu       sync.RWMutex
	// Values 的 key 为 measures + tags + filedName，值就是一个时间戳对应的实际值
	values   Values // All stored values.

	// 记录 values 中的数据是否需要重新排序
...
func (e *entry) add(values []Value) {
	...
	// 同样是检查，如果新插入的数据的时间范围和之前的数据有重叠，需要重新排序
	e.mu.Lock()
	if needSort {
		e.needSort = needSort
	}
