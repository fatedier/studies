...
type StatementExecutor struct {
	...
	// 如果查询请求涉及到 查询任务执行的信息，通过 TaskManager 来查询
...
func (e *StatementExecutor) executeSelectStatement(stmt *influxql.SelectStatement, ctx *influxql.ExecutionContext) error {
	...
	// 如果是 show tag values 的请求，优化速度而特殊处理
...
func (e *StatementExecutor) iteratorCreator(stmt *influxql.SelectStatement, opt *influxql.SelectOptions) (influxql.IteratorCreator, error) {
	...
	// 根据查询语句获取所要获取的数据所在的所有 shard 的信息
	// 主要是根据数据库名，存储策略，measurements，以及时间范围
...
// 查询 tags 相关的信息
// SHOW TAG KEYS FROM "sys.cpu.usage.all"
// SHOW TAG VALUES FROM "sys.cpu.usage.all" WITH KEY = "host"

// 转换成 SELECT 语句后
// SELECT tagKey FROM common.autogen._tagKeys WHERE _name = 'sys.cpu.usage.all' 
// SELECT _tagKey AS "key", value FROM common.autogen._tags WHERE (_name = 'sys.cpu.usage.all') AND (_tagKey = 'host')
func (e *StatementExecutor) executeShowTagValues(stmt *influxql.SelectStatement, ctx *influxql.ExecutionContext, store LocalTSDBStore) error {
	...
	if err != nil {
		...
		// 如果没有 measurements 的过滤条件，获取所有的 measurements
	...
	var emitted bool
	// 获取查询语句中涉及到的所有 fieldName
	columns := stmt.ColumnNames()
	for _, mm := range mms {
		// 根据表达式获取匹配的所有 series 的 ID
		ids, err := mm.SeriesIDsAllOrByExpr(filterExpr)
		if err != nil {
			return err
		}
		// 根据 seriesID 获取 series 对象
