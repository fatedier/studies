...
type Mux struct {
	mu sync.RWMutex
	ln net.Listener				// 真正用于建立连接的 listener
	m  map[byte]*listener		// 注册的 handler，根据连接建立后读取到的第一个字节来分发连接
...
// 传入一个用于真正接收连接的 listener，循环等待连接的建立，根据连接建立后的第一个字节来分发连接
...
// 通过建立连接后的一个字节判断出是哪一个已经注册的 handler 来处理这个连接，之后将这个连接交给指定的 handler 去处理
func (mux *Mux) handleConn(conn net.Conn) {
	...
	// 通过第一个字节判断出是什么服务类型，之后分发到不同的 listener 中去
	var typ [1]byte
...
// 注册一个用于 handler 连接的 listener，通过一个字节来进行分发
