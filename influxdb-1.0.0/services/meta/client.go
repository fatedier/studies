...
type Client struct {
	...
	retentionAutoCreate bool	// 是否自动创建默认的 RP
}
...
func (c *Client) Open() error {
	...
	for _, db := range c.cacheData.Databases {
		fmt.Printf("load metadata db [%s]\n", db.Name)
	}
...
// 指定数据库以及存储策略，时间范围，获取可能存有对应数据的所有 shardGroup 信息
...
// 指定 measurement，RP，以及时间范围，获取对应的所有 shards 信息
