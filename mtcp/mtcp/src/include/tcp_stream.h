...
#define MAX_SACK_ENTRY 8
	...
	struct tcp_ring_buffer *rcvbuf;		// tcp接收缓存
#if USE_SPIN_LOCK
...
typedef struct tcp_stream
{
	...
	uint32_t saddr;			/* in network order */	// 32位本机ip地址
	uint32_t daddr;			/* in network order */	// 32位目的ip地址
	uint16_t sport;			/* in network order */	// 16位本机端口号
	uint16_t dport;			/* in network order */	// 16位目的端口号
