...
{
	// iph->ihl为ip头部长度，首部长度指的是IP层头部占32 bit字的数目(也就是IP层头部包含多少个4字节 -- 32位)，需要乘以4

	// 获取tcp首部
	struct tcphdr* tcph = (struct tcphdr *) ((u_char *)iph + (iph->ihl << 2));
	// tcph->doff为tcp头部长度，表示32bit字的数目
	// payload为具体数据内容的地址
	...
	s_stream.saddr = iph->daddr;	// 32位目的ip地址，对于本机来说相反
	s_stream.sport = tcph->dest;	// 16位目的端口号，对于本机来说相反
	s_stream.daddr = iph->saddr;	// 32位源ip地址，对于本机来说相反
	s_stream.dport = tcph->source;	// 16位源端口号，对于本机来说相反

	// 在哈希表中查找指定的tcp_stream
