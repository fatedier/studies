...
struct dpdk_private_context {
	struct mbuf_table rmbufs[RTE_MAX_ETHPORTS];		// 每个网卡对应的接收包mbuf
	struct mbuf_table wmbufs[RTE_MAX_ETHPORTS];		// 每个网卡对应的发送缓冲区
	struct rte_mempool *pktmbuf_pool;				// 包管理内存池
	struct rte_mbuf *pkts_burst[MAX_PKT_BURST];		// 数据包的mbuf
#ifdef ENABLE_STATS_IOCTL
	int fd;											// dpdk网卡文件描述符
#endif /* !ENABLE_STATS_IOCTL */
} __rte_cache_aligned;
...
dpdk_load_module(void)
{
	...
	if (!CONFIG.multi_process || (CONFIG.multi_process && CONFIG.multi_process_is_master)) {
		...
		for (portid = 0; portid < num_devices_attached; portid++) {
			...
			printf("done: \n");
			// 判断你的网卡设置的是否混咋模式接收信息，如果是混杂模式，说明你任何网络信息都接收，否则只接收你上面设置的mac地址为目的地址的网络信息
